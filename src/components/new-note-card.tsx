...
export function NewNoteCard({ onNoteCreated }: NewNoteProps) {
  ...
  return (
    <Dialog.Root open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <Dialog.Trigger className="relative flex flex-col gap-3 overflow-hidden rounded-3xl bg-zinc-200 p-5 text-left outline-none transition hover:ring-2 hover:ring-red-600 focus-visible:ring-2 focus-visible:ring-red-600 dark:bg-zinc-600">
        <span className="flex gap-2 text-lg font-medium text-zinc-900 md:text-xl dark:text-zinc-100">
          <FilePlus2 />
          Adicionar relatoria
        </span>
        <p className="text-md leading-6 text-zinc-700 md:text-lg dark:text-zinc-300">
          ...
          <Dialog.Content className="fixed inset-0 flex w-full flex-col overflow-hidden bg-zinc-300 outline-none md:inset-auto md:left-1/2 md:top-1/2 md:h-[80vh] md:max-w-[120vh] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-3xl dark:bg-zinc-700">
            ...
            <form className="flex flex-1 flex-col">
              <div className="flex flex-1 flex-col justify-center gap-3 p-5 md:justify-start">
                <span className="flex gap-2 text-2xl font-medium text-zinc-900 md:text-lg dark:text-zinc-300">
                  <FilePlus2 />
                  Adicionar relatoria
                </span>
                {shouldShowOnboarding ? (
                  <p className="leading-12 text-xl text-zinc-700 md:text-lg md:leading-6 dark:text-zinc-400">
                    Comece{" "}
                    <button
                      type="button"
              ...
              {isRecording ? (
                <button
                  ...
                  className="text-md flex w-full items-center justify-center gap-2 bg-zinc-600 py-4 text-center text-xl font-medium text-zinc-100 outline-none transition hover:text-xl hover:text-red-500 md:text-lg dark:bg-zinc-900 dark:text-zinc-300"
                >
                ...
                <button
                  ...
                  className="text-md flex w-full items-center justify-center gap-2 bg-zinc-950 py-4 text-center text-xl font-medium text-zinc-100 outline-none transition hover:text-xl hover:text-green-500 md:text-lg dark:bg-zinc-900"
                >
