...
export function NoteCard({ note }: NoteCardProps) {
    return (
        <Dialog.Root>
            <Dialog.Trigger
                className="rounded-md text-left flex flex-col bg-slate-800 p-5 gap-3 overflow-hidden relative outline-none hover:ring-2 hover:ring-purple-600 focus-visible:ring-2 focus-visible:ring-purple-400">
                ...
                <Dialog.Overlay className='inset-0 fixed bg-black/60'>
                    <Dialog.Content className="fixed overflow-hidden left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] w-full h-[60vh] bg-slate-700 rounded-md flex flex-col">
                        ...
                        <div className="flex flex-1 flex-col gap-3 p-5">
                            ...
                            <p className="text-xl leading-6 text-slate-400">{note.content}</p>
                        </div>

                        <button
                            type="button"
                            className="w-full bg-slate-800 py-4 text-center text-xl text-slate-300 outline-none font-medium group"
                        >
